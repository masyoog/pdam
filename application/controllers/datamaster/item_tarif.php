 ...
 */
class Item_tarif extends MY_Controller {
    
    private $_CFG;
    private $_TBL_PRIMARY = "mt_item_tarif a";
    private $_TBL_PRIMARY_PK = "a.id";
    private $_ORDER = array("a.urutan, a.kode, a.nama ASC");
    ...
    function __construct() {
        ...
        $menu->set_REQUIRED(TRUE);
        $menu->set_SIZE(128);
        $this->_CFG->add_column("Nama Item Tarif", $menu);

        $status = new Datagridcolumn();
        $status->set_FIELD_DB("a.tipe_tarif");
        $status->set_FIELD_TYPE($status->get_ENUM_TYPE());
        $status->set_FORM_ID("tipe_tarif");
        ...
        $this->_CFG->add_column("Jenis Tarif", $status);

        $status = new Datagridcolumn();
        $status->set_FIELD_DB("a.is_rutin");
        $status->set_FIELD_TYPE($status->get_ENUM_TYPE());
        $status->set_FORM_ID("is_rutin");
        $status->set_ENUM_DEFAULT_VALUE(
                array("1" => "Ya",
                    "0" => "Tidak"
        ));
        $status->set_STYLE(
                array("1" => '<span class="label label-success">Ya</span>',
                    "0" => '<span class="label label-warning">Tidak</span>'
        ));
        $status->set_SIZE(1);
        $this->_CFG->add_column("Biaya Rutin", $status);

        $menu = new Datagridcolumn();
        $menu->set_FIELD_DB("a.urutan");
        $menu->set_FORM_ID("urutan");
        $menu->set_REQUIRED(TRUE);
        $menu->set_FIELD_TYPE($menu->get_NUM_TYPE());
        $menu->set_SIZE(2);
        $this->_CFG->add_column("Urutan", $menu);
        ...
        if ($this->input->post('save') != "") {
            ...
            if ($errorMsg == "") {
                ...
                if ( "" != $this->base_model->db->_error_message()){
                    ...
                    redirect(base_url($this->_INDEX_PAGE."/index/") . _build_query_string($this->_get_query_string()));
                }
            }
