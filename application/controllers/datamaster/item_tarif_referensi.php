 ...
 */
class Item_tarif_referensi extends MY_Controller {
    
    private $_CFG;
    private $_TBL_PRIMARY = "mt_item_tarif_referensi a";
    private $_TBL_PRIMARY_PK = "a.id";    
    private $_ORDER = array("a.id ASC");
    private $_ITEM_PER_PAGE = "";
    private $_TBL_JOIN = array(
        "mt_item_tarif b"=>"b.id=a.id_item_tarif"
    );
    private $_INDEX_PAGE;
    ...
    function __construct() {
        ...
        $status->set_FIELD_DB("a.tipe_referensi");
        ...
        $this->_CFG->add_column("Tipe Referensi", $status);

        
        $menu = new Datagridcolumn();
        $menu->set_FIELD_DB("a.referensi");
        $menu->set_SIZE(128);
        $menu->set_FORM_ID("referensi");
        $menu->set_REQUIRED(TRUE);
        $this->_CFG->add_column("Referensi", $menu);
    ...
    function form($mode="add", $key = "") {
        ...
        $rsTipePelanggan = $this->base_model->list_data(
                "a.id as kunci, (a.kode ||' - '|| a.nama) as nilai", "mt_item_tarif a", "",array("status"=>1, "tipe_tarif"=>"PAJAK"), array("a.kode asc"));
        ...
        $tipe_pelanggan->set_VALIDATION("callback_select_required");
