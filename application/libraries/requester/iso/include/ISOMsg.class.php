...
class ISOMsg {
    ...
    private function _packElement($data_element, $data) {
        ...
        //numeric value
        if ($data_element[0]=='n' && strlen($data)<=$data_element[1]) {
        ...
        //alpha value
        if (($data_element[0]=='a' && strlen($data)<=$data_element[1]) ||
            ($data_element[0]=='an' && strlen($data)<=$data_element[1]) ||
    ...
    public function getData() {
		...
		// foreach($this->_data as $key => $val){
			// $ret[$key]=$val;
			// if($this->DATA_ELEMENT[$key][2]==1){
				// $ret[$key]=substr($ret[$key], strlen($this->DATA_ELEMENT[$key][1]));
			// }
		// }
        // return $ret;
		return $this->_data;
